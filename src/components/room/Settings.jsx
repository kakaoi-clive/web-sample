              ...
              <div className="relative inline-block bg-white rounded-lg text-left overflow-hidden shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full text-gray-700">
                <form className="bg-white px-4 pt-5 pb-4">
                  ...
                  <div className="col-span-6 sm:col-span-3 mb-6">
                    ...
                    <select className="w-full py-2 px-3 border border-gray-300 bg-white text-black rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" value={cameraDeviceId} onChange={onChangeCameraDevice}>
                      {
                        cameraDevices.map((camera, i) => {
                          return (<option key={i} value={camera.deviceId}>{camera.label}</option>);
                        })
                      }
                    </select>
                  </div>
                  <div className="col-span-6 sm:col-span-3 mb-6">
                    ...
                    <select className="w-full py-2 px-3 border border-gray-300 bg-white text-black rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" value={micDeviceId} onChange={onChangeMicDevice}>
                      {
                        micDevices.map((mic, i) => {
                          return (<option key={i} value={mic.deviceId}>{mic.label}</option>);
                        })
                      }
                    </select>
                  </div>
                  <div className="col-span-6 sm:col-span-3 mb-6">
                    ...
                    <select className="w-full py-2 px-3 border border-gray-300 bg-white text-black rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" value={speakerDeviceId} onChange={onChangeSpeakerDevice}>
                      {
                        speakerDevices.map((speaker, i) => {
                          return (<option key={i} value={speaker.deviceId}>{speaker.label}</option>);
                        })
                      }
                    </select>
                  </div>
