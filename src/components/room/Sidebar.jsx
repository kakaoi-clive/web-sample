...
const Sidebar = ({ sidebarOpen, setSidebarOpen, remoteParticipants, onRemoteVideoIdClick }) => {
  ...
  return (
    <div>
      ...
      {/* Sidebar */}
      <div
        className={`flex flex-col absolute z-40 left-0 top-0 lg:static lg:left-auto lg:top-auto lg:translate-x-0 transform h-screen overflow-y-auto lg:overflow-y-auto no-scrollbar w-64 lg:w-20 lg:sidebar-expanded:!w-64 2xl:!w-64 bg-slate-800 p-4 transition-all duration-200 ease-in-out ${
                                  ...
                                  >
                                    {remoteVideo.active ? (
                                      <span className='text-sm font-bold text-white'>
                                        {remoteVideo.getExtraValue()}
                                      </span>
                                    ) : (
                                      <span className='text-sm font-medium text-slate-400 opacity-40'>
                                        {remoteVideo.getExtraValue()}
                                      </span>
                                    )}
                                  </div>
